...
def solve_with_ele(positions, minutes_left, opened_pipes, next_released_preasure):
    # print (position, minutes_left, opened_pipes, realesed_preasure, next_released_preasure)
    key = (tuple(positions), minutes_left, opened_pipes)
    if key in mem:
        return mem[key]

    # print(cnt)
    # print(opened_pipes, ((2 << cnt) -1), opened_pipes == (2 << cnt) -1)
    if opened_pipes == ((2 << cnt) -1) :
        res = next_released_preasure * minutes_left
        # print (positions, minutes_left, opened_pipes, next_released_preasure, res)
        mem[key] = res
        return res
    ...
    # opposite = positions[1]
    if ((((2 << R[positions[0]]) & opened_pipes) == 0) and rates[positions[0]] > 0
        and positions[0] != positions[1]
        and ((2 << R[positions[1]]) & opened_pipes) == 0) and rates[positions[1]] > 0:
            ns = opened_pipes | (2 << R[positions[0]]) | (2 << R[positions[1]])
            # ns = set(list(opened_pipes) + [position])
            results.append(solve_with_ele(positions, minutes_left-1, ns, next_released_preasure + rates[positions[0]] + rates[positions[1]]))
    elif (((2 << R[positions[0]]) & opened_pipes) == 0) and rates[positions[0]] > 0:
        ns = opened_pipes | (2 << R[positions[0]])
    ...
    elif (((2 << R[positions[1]]) & opened_pipes) == 0) and rates[positions[1]] > 0:
        ns = opened_pipes | (2 << R[positions[1]])
...
for i in range(15):
    mem = {}
    print(i+1, solve_with_ele(['AA','AA'], i+1, 0, 0))

# print ((2 << cnt))
# print(solve_with_ele(['AA','AA'], 26, 0, 0))
# print (pp)
